...
class BOMReview {
  ...
  async loadData() {
    ...
    // Check if BOM was pre-generated by Configure Build page
    if (this.bomData?.bomResult?.lineItems) {
      console.log('[BOM Review] Using pre-generated BOM data');
      this.bomData.lineItems = this.convertMeshBomToLineItems(this.bomData.bomResult);
      this.bomData.totalCost = this.bomData.bomResult.totalCost;
      return; // Data is ready, no loading needed
    }
    
    // Fallback: Generate BOM here if not pre-generated (legacy flow)
    if (this.bomData?.templateId && this.bomData?.selectedPackage) {
      console.log('[BOM Review] BOM not pre-generated, loading now...');
