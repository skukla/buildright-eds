 ...
 * Called by scripts.js after dropins are initialized
 * @param {Document} doc - The document (optional, defaults to window.document)
 * @returns {Promise<void>}
 */
export async function personalizeHomepage(doc = document) {
  ...
  if (isAuth) {
    ...
    const unauthSections = doc.querySelectorAll('.unauthenticated-only');
    unauthSections.forEach(section => {
      section.style.display = 'none';
    });

    // Update fragment blocks with personalized paths
    const heroFragment = doc.querySelector('.hero-fragment');
    const featuresFragment = doc.querySelector('.features-fragment');
    const ctaFragment = doc.querySelector('.cta-fragment');
 ...
 */
function personalizeWithDynamicData(persona, doc = document) {
  // Example: Update name dynamically (first name only)
  const nameElements = doc.querySelectorAll('[data-persona-name]');
  ...
  const companyElements = doc.querySelectorAll('[data-persona-company]');
  companyElements.forEach(el => {
    el.textContent = persona.company;
  });

  // Example: Update role dynamically
  const roleElements = doc.querySelectorAll('[data-persona-role]');
