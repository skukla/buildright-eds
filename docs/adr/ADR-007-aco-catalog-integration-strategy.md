# ADR-007: ACO Catalog Integration Strategy

**Status**: Accepted (Revised January 2026)

**Date**: December 2025 (Original), January 2026 (Revised)
...
BuildRight products live in **ACO**, not Commerce. The question was: how do we display ACO products using the Commerce dropin ecosystem?
...
4. **Mesh adapter pattern** — Use API Mesh to intercept dropin queries and route to ACO

---

## Decision

**We use native Commerce dropins with API Mesh adapters to query ACO.**

> **Note**: The original decision (December 2025) proposed building custom SDK dropins. During implementation, we discovered that native dropins work with ACO when the mesh provides proper headers. See "Implementation Status" section below for details.
...
- [master-implementation-plan.md](../planning/master-implementation-plan.md) — Overall implementation plan
