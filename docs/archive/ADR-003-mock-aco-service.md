...
**Status**: Superseded (Archived January 2026)

**Date**: 2025-11-15

**Superseded By**: Real API Mesh → ACO integration (December 2025)

**Decision Makers**: BuildRight Implementation Team

---

> **⚠️ ARCHIVED**: This ADR describes the mock ACO service (`scripts/aco-service.js`) that was used during initial development before the real API Mesh integration was ready. The mock served its purpose and has been superseded.
>
> **Current Architecture** (supersedes this ADR):
> - Frontend → API Mesh → ACO (real connection)
> - Persona headers (`AC-View-Id`, `AC-Price-Book-Id`) enable persona-specific catalogs
> - See `buildright-service/mesh/README.md` for mesh architecture
>
> The `scripts/aco-service.js` file still exists as a deprecated fallback but is no longer the primary data source.
